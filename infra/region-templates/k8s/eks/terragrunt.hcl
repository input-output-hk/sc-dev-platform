locals {
  ...
  bootstrap_cmd = <<-EOT
        "max-pods" = ${run_cmd("/bin/sh", "-c", "AWS_REGION=${local.region} AWS_PROFILE=${local.profile} ${get_parent_terragrunt_dir()}/modules/scripts/max-pods-calculator.sh --instance-type t3a.large --cni-version 1.11.4")}
        EOT
...
inputs = {
  ...
  eks_managed_node_group_defaults = {
    ...
    ami_release_version          = "1.10.1-5d27ae74"
